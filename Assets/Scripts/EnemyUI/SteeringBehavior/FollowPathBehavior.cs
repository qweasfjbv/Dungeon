...
{
    [SerializeField] public PathLine path;
    ...
    public override SteeringData GetSteering(SteeringBehaviorBase steeringBase)
    {
        ...
        else
        {
            ...
            steering.velocity = targetPosition - transform.position;
            steering.velocity.Normalize();
            steering.velocity *= steeringBase.maxVelocity;
        }
        return steering;
...
public class PathLine
{
    ...
    public void SetNodes(List<Vector2> list)
    {
        nodes = new Vector3[list.Count];

        for (int i = 0; i < list.Count; i++)
        {
            nodes[i] = new Vector3(list[i].x, list[i].y, 0);
        }
